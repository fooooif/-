#
# SELDnet을 훈련시키는 래퍼 스크립트.  조기 중지 메트릭-SELD 오류가 개선되지 않으면 훈련이 중지됩니다.
#
...
'''
    ...
    plot.plot(range(nb_epoch), _new_metric[:, 3], label='doaf r')
...
def main(argv):
    """
    사운드 이벤트 위치 파악 및 감지 네트워크 훈련을위한 기본 래퍼입니다.
    ...
    for split_cnt, split in enumerate(test_splits):
        ...
        for epoch_cnt in range(nb_epoch):
            ...
            # DCASE 2019 점수 계산 - Detection-only and Localization-only scores
            ...
            # DCASE 2020 점수 계산 - Location-aware detection and Class-aware localization scores
            ...
            print(
                'epoch_cnt: {}, time: {:0.2f}s, tr_loss: {:0.2f},'.format(
        ...
        if params['dcase_output']:
            # 최종 점수 계산을 위해 DCASE 출력 형식으로 결과 덤프
            ...
            # 한 배치의 프레임 수 (batch_size * sequence_length)는 위의 모든 600 개 프레임으로 구성되며
            # 나머지 프레임에서 제로 패딩
