# SELD 출력을 시각화하기위한 스크립트.
#
# 참고 : OS에 따라 matplotlib에 적절한 백엔드를 사용
...
results_csv = os.path.join(params['dcase_dir'],'1_mic_dev') #파일 경로 합치기
list_results_csv = os.listdir(results_csv) # 경로 안에 있는 파일 확인

for i in list_results_csv:

    # output format file to visualize
    pred = os.path.join(results_csv, i)
    ...
    ax2 = plot.subplot(gs[1, 2:]), plot_func(pred_data, params['label_hop_len_s'], ind=1, plot_y_ax=True), plot.ylim( [-1, nb_classes + 1]), plot.title('SED predicted')
    ...
    ax4 = plot.subplot(gs[2, 2:]), plot_func(pred_data, params['label_hop_len_s'], ind=2, plot_y_ax=True), plot.ylim( [-180, 180]), plot.title('Azimuth predicted')
    ...
    ax6 = plot.subplot(gs[3, 2:]), plot_func(pred_data, params['label_hop_len_s'], ind=3, plot_y_ax=True), plot.ylim( [-90, 90]), plot.title('Elevation predicted')
